...
&lt;iframe
  src="https://akosileyoruba.github.io/Atupaleyoruba?embed=1&amp;level=1"
  width="100%"
  height="560"
...
<section class="panel" id="p-src">
  ...
  <div style="font-size:11.5px;color:var(--ink-soft);margin-top:8px;line-height:1.5;">
    ...
    Share this app in vibe coder communities (Reddit, X, Facebook dev groups) as an example of an AI-built app ready for Pi App Studio. Then submit your post link via the <b>"Vibe Coder" button</b> in your Pi mining app.<br><br><b>Your live app URL:</b> <a href="https://akosileyoruba.github.io/Atupaleyoruba" style="color:var(--gold)" target="_blank">https://akosileyoruba.github.io/Atupaleyoruba</a>
  </div>
</section>
<!-- LANGUAGE -->
...
// Implements Pi App Studio Step 2/2 requirements:
// - Await Pi.init() as a Promise before Pi.authenticate()
// - Auto-trigger on load + manual sign-in button
// - Use "username" scope
// - Validate access token via GET https://api.minepi.com/v2/me
...
async function initPi(){
  ...
  const nameEl = document.getElementById('pi-username');
  const statEl = document.getElementById('pi-status');
  ...
  if(!isPiBrowser()){
    nameEl.textContent = 'Open in Pi Browser';
    statEl.textContent = 'Pi features require the Pi Browser app';
    bar.style.background = '#5c5341';
    if(signInBtn) signInBtn.style.display = 'none';
    return;
  }

  nameEl.textContent = 'Connecting to Pi...';
  statEl.textContent = 'Initialising...';

  try{
    // Step 1: Await Pi.init() as a Promise before doing anything else
    await Pi.init({ version: '2.0', sandbox: false });

    // Step 2: Authenticate with "username" scope
    const scopes = ['username', 'payments'];

    function onIncompletePaymentFound(payment){
      console.log('Incomplete payment found — handling:', payment.identifier);
      // Required callback: acknowledge to Pi SDK so it doesn't block auth
    }
    ...
    // Step 3: Validate token by calling Pi's /v2/me endpoint
    // (This is the server-validation step Pi App Studio requires)
    try{
      ...
      if(meRes.ok){
        ...
        // Confirmed: token is valid, user is authenticated
        piUser = meData; // use verified data from Pi's server
        console.log('Pi user verified:', piUser.username);
      } else {
        console.warn('Token validation response:', meRes.status, '— using local auth data');
      }
    }catch(fetchErr){
      // CORS will block this from GitHub Pages — expected in frontend-only apps.
      // The local auth object is still valid for Pi Browser in-app usage.
      console.warn('Token validation fetch blocked (expected on GitHub Pages):', fetchErr.message);
    }

    piReady = true;
    const displayName = (piUser && piUser.username) ? piUser.username : auth.user.username;
    nameEl.textContent = 'π ' + displayName;
    statEl.textContent = 'Signed in · ÀTÚPALẸ̀ Yorùbá';
    if(signInBtn) signInBtn.style.display = 'none';
    ...
    statEl.textContent = 'Tap "Sign in with Pi" to connect';
...
async function piPayForPuzzle(){
  ...
  const paymentData = {
    ...
    metadata: { app: 'atupale-yoruba', feature: 'puzzle-session',
                 user: piUser ? piUser.username : 'unknown', timestamp: Date.now() }
  };
  const callbacks = {
    onReadyForServerApproval: function(paymentId){
      console.log('Ready for approval, paymentId:', paymentId);
      // For a full backend: POST paymentId to your server to call Pi's approve endpoint
    },
    onReadyForServerCompletion: function(paymentId, txid){
      console.log('Payment complete! txid:', txid);
      unlockPuzzleSession();
    },
    onCancel: function(paymentId){ console.log('Payment cancelled:', paymentId); },
    onError: function(error, payment){
      console.error('Payment error:', error, payment);
      alert('Payment error: ' + (error.message || error));
    }
  };
  try{ await Pi.createPayment(paymentData, callbacks); }
  catch(e){ console.error('createPayment error:', e); }
}
...
// Wire up buttons and auto-init on load
document.addEventListener('DOMContentLoaded', function(){
  const payBtn = document.getElementById('pi-pay-puzzle');
  const signInBtn = document.getElementById('pi-signin-btn');
  if(payBtn) payBtn.addEventListener('click', piPayForPuzzle);
  if(signInBtn) signInBtn.addEventListener('click', initPi);
  // Auto-trigger Pi authentication on page load (Pi App Studio requirement)
  initPi();
});
